...
export const DeploymentForm = ({ deployment }: DeploymentFormProps) => {
	...
	const form = useDeploymentForm(deployment);
	const watchPoolName = form.watch("work_pool_name");

	const { updateDeployment } = useUpdateDeployment();

	const handleUpdateDeployment = (values: DeploymentFormSchema) => {
		const {
			job_variables,
			description,
			work_pool_name,
		...
		updateDeployment(
			{
				...
				concurrency_limit,
				concurrency_options,
				job_variables: void JSON.parse(job_variables),
			},
			{
				onSuccess: () => {
	...
	return (
		<Form {...form}>
			<form
				onSubmit={(e) => void form.handleSubmit(handleUpdateDeployment)(e)}
				className="space-y-4"
			>
